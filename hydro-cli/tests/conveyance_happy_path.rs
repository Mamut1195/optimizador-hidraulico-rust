//! Integration test: run() conveyance dispatch (REQ-001, REQ-004).
//!
//! ## Strict TDD
//!
//! RED commit: `run()` returns `Err(CliError::InternalError("... WU-5"))` for
//! `project_type = "conveyance"`. The `expect` below panics, satisfying RED gate.
//!
//! GREEN commit: `run()` conveyance arm is implemented; all assertions pass.
//!
//! ## What is tested
//!
//! Builds a `DesignRequest` from the conveyance golden fixture with a fast GA
...
use oracle::solvers::load_conveyance_golden;

/// Happy-path: run() dispatches to ConveyanceSolver and returns success (REQ-001, REQ-004).
///
/// Uses the conveyance golden fixture for terrain + source/destination topology.
/// GA budget: pop=20, gen=10, seed=42 — the validated floor per validate_request.
#[test]
fn test_run_conveyance_dispatch() {
    ...
    // For Conveyance: source = pipeline origin, outlet = destination.
    let source = PointXY {
        x: f.solver_params.source[0],
        y: f.solver_params.source[1],
    };
    let outlet = PointXY {
        x: f.solver_params.destination[0],
        y: f.solver_params.destination[1],
    };

    let req = DesignRequest {
        project_type: ProjectTypeStr("conveyance".to_string()),
        project_name: "test_conveyance_wu5".to_string(),
        terrain_points,
        service_points: None, // not needed for conveyance
        source: Some(source),
        outlet: Some(outlet),
        ...
        // flow_per_service doubles as design_flow for Conveyance (design §2.5).
    ...
    let result = run(req, Some(42)).expect("run() must return Ok for a valid conveyance request");

    // ASSERT — REQ-001 happy path scenario.
    assert!(
        result.success,
        "DesignResult.success must be true for a valid conveyance request"
    );
    assert!(
